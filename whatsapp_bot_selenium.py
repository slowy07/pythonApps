...
def name_input():
    user_name = input("who's do you want to send the message ?")
    chatInput = driver.find_element_by_xpath(
...
##TEXT LANE
def texter():
    lane = driver.find_element_by_xpath(
...
def reader():
    args = driver.find_element_by_class_name(
...
def last_text():
    elem = driver.find_element_by_xpath(
        '//*[@id="main"]//div[contains(@class, "message")]'
    )
    print(elem.text)
...
##ATTACHMENT
def attachment():
    button_a = driver.find_element_by_xpath(
...
def status():
    profile_self()
    status_point = driver.find_elements_by_css_selector(
        "div._2S1VP.copyable-text.selectable-text"
    )
...
def forwarder():
    ...
    while choicer != 1:
        ...
        iol += 1
        choicer = format_checker(
            "press 1 to exit, press any other key to continue adding list"
        )
    ...
    def ex_forwarder():
        c_1 = format_checker(
            "Press 1 to add more contact || press 2 to delete some contacts || press 3 to continue"
        )
        if c_1 == 1:
            ...
            list_print()
            namer_t = input(
                "Enter the Name of the person to remove!! please be accurate"
            )
            kr = 0
            for y in range(iol):
    ...
    def execute_forwarder():
        ...
        for xoz in range(iol):
            ...
            opener.click()
            lane_rn = driver.find_element_by_xpath(
                '//*[@id="main"]/footer/div[1]/div[2]/div/div[2]'
            )
            izp = 0
            while izp != izr:
...
def actual_exe():
    print(
        "PRESS 1 FOR MULTIPLE SENDER || PRESS 2 FOR FORWARDER || PRESS 3 TO VIEW YOUR OWN PIC || PRESS 4 TO STATUS || PRESS 5 TO EXIT"
    )
    c_4 = format_checker(" ")
    if c_4 == 1:
        multiple_sender()
